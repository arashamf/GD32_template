...
{   
	...
	ssd1306_SendCommand(SETDISPLAYCLOCKDIV);
	ssd1306_SendCommand(0x80);

	ssd1306_SendCommand(SETMULTIPLEX); 
	ssd1306_SendCommand(0x3F); //32 (128x32)

	ssd1306_SendCommand(SETDISPLAYOFFSET); //-set display offset
	ssd1306_SendCommand(0x00); //-not offset

	ssd1306_SendCommand(SETSTARTLINE | 0x00); 

	ssd1306_SendCommand(0x8D); //set pump voltage value
	...
	ssd1306_SendCommand(SETSEGREMAP+0x00); //set segment re-map 0 to 127, 0x01 - left rotation, 0x00 - right rotation 	
	ssd1306_SendCommand(COMSCANDEC); //Set COM Output Scan Direction. 0xC8: remapped mode
	...
	ssd1306_SendCommand(DISPLAYALLON_RESUME); 
...
{
		CS(ON);
		SSD1306_WRITECOMMAND(0xB0 + i); //��������� ������ X[2:0] ������ ��������� �������� RAM (PAGE0~PAGE7) ��� Page Addressing Mode
